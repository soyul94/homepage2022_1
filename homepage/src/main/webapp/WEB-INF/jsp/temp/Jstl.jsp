...
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %> <!-- core태그 라이브러리를 사용하기 위한 taglib디렉티브 태그 -->
<%@ taglib prefix="ui" uri="http://egovframework.gov/ctl/ui" %>
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions"  %><!-- function태그 라이브러리를 사용하기 위한 taglib디렉티브 태그 -->
...
<body>
	...
	<ul>
		...
		<c:when test="${step eq '2'}">
			<li>
				<h3>c:import Tag : jsp 'include'와 유사. 서버 내부 또는 서버 외부의 모든 자원 컨덴츠를  포함하는 추가 기능이 있음</h3>
		...
		<c:when test="${step eq '5'}">
			<li>
				<h3>c:choose, c:when, c:otherwise Tag : 조건문 중의 하나</h3>
				<h3>JAVA에서 사용하는 if, else if, else와 비슷함</h3>
				<h4>기본문법 : <br/>
					&#60;c:choose&#62;<br/>
					&#09;&#60;c:when test="조건문"&#62; 조건 true에 대한 내용 &#60;/c:when&#62;<br/>
					&#09;&#60;c:otherwise&#62; 조건에 해당되지 않을 때에 대한 내용&#62;/c:otherwise&#62; 생략가능<br/>
					&#60;/c:choose&#62;
				</h4>
		...
		<c:when test="${step eq '7'}">
			<li>
				<h3>c:param Tag : 포함하는 impor, url 태그의 URL에 매개 변수를 추가</h3>
		...
		<c:when test="${step eq '8'}">
			<li>
				...
				URL 2 : <c:url var="link2" value="/temp/jstlImport.do"> <%-- title 태그는 시각장애인들을 위한 내부적 태그--%>
							<c:param name="test" value="테스트 호출"/>
						</c:url>
						<a href="${link2}" target="_blank">링크 2번</a>
						<br/> 		   <%--target="_blank"는 새창으로 열어준다 --%>
			</li>
		</c:when>
		...
		<c:when test="${step eq '9'}">
			<li>
				...
				<p>현재 주어진 문장 "지금은 fn:contains 연습중" 으로 <br>fn:contains(str,'지금')을 수행한 결과 : 
				<c:set var="str" value="지금은 fn:contains 연습중"/>
				<c:if test="${fn:contains(str,'지금')}">true</c:if></p>
			</li>
		</c:when>
		<c:when test="${step eq '10'}">
			<li>
				<h3>fn:indexOf() : 주어진 문자열에서 특정 문자열이 어느 위치에 있는지 반환</h3>
				
				<c:set var="string" value="It is first String."/>
				<p>Index : ${fn:indexOf(string,"first")}</p>
				<p>이 때 위치를 확인할 문자열은 스펠링이 완전하게 동일해야 문자열의 위치를 찾을 수 있다.</p>
			</li>
		</c:when>
		<c:when test="${step eq '11'}">
			<li>
				<h3>fn:startsWith() : 주어진 문자열이 특정문자열 값으로 시작되는지 확인하는 데 사용</h3>
				
				<c:set var="String" value="Welcome to JSP programming"/>
				<p>set var="String" value="Welcome to JSP programming"</p>
				<p>fn:startsWith(String,'Welcome') 의 결과: <c:out value="${fn:startsWith(String,'Welcome')}" /></p>
				<c:if test="${fn:startsWith(String,'Welcome')}">
					<p>String starts with Welcome</p>
				</c:if>
				<p>이 때  확인할 문자열은 스펠링이 완전하게 동일해야 시작한다고 판단한다.</p>
			</li>
		</c:when>
		<c:when test="${step eq '12'}">
			<li>
				<h3>fn:endsWith() : 주어진 문자열이 특정 문자열 값으로 끝나는지 확인하는 데 사용</h3>
				
				<c:set var="String" value="Welcome to JSP programming"/>
				<p>set var="String" value="Welcome to JSP programming"</p>
				<p>fn:endsWith(String,'programming') 의 결과: <c:out value="${fn:endsWith(String,'programming')}" /></p>
				
				<c:if test="${fn:endsWith(String,'programming')}">
					<p>String ends with programmig</p>
				</c:if>
		...
		<c:when test="${step eq '13'}">
			<li>
				<h3>fn:split() : 주어진 문자열을 특정 문자로 구문해서 배열로 분할</h3>
				<p>set var="str" value="1,2,3,4,5,6"을 fn:split(str,',')한 결과 : </p>
				<c:set var="str" value="1,2,3,4,5,6"/>
				<c:set var="strSplit" value="${fn:split(str,',')}"/>
				<c:forEach var="result" items="${strSplit}">
					<c:out value="${result}"/><br>
		...
		<c:when test="${step eq '14'}">
			<li>
				...
				<c:set var="strSplit" value="${fn:split(str2,',')}"/>
				
				Length of the String_1 is : <c:out value="${fn:length(str1)}"/><br>
				Length of the String_2 is : <c:out value="${fn:length(strSplit)}"/>
			</li>
		</c:when>
		<c:when test="${step eq '15'}">
			<li>
				...
				<c:set var="string" value="Welcome to JSP programming"/>
				<c:out value="fn:toLowerCase(\"Welcome to JSP programming\")= ${fn:toLowerCase(string)}"/>
			</li>
		</c:when>
		<c:when test="${step eq '16'}">
			<li>
				...
				<c:set var="string" value="Welcome to JSP programming"/>
				<c:out value="fn:toUpperCase(\"Welcome to JSP programming\")= ${fn:toUpperCase(string)}"/>
			</li>
		</c:when>
		<c:when test="${step eq '17'}">
			<li>
				...
				<c:out value="fn:substring(\"2022-01-01\", 0, 4) = ${fn:substring(string,0,4)}"/><br>
				<c:out value="fn:substring(\"2022-01-01\", 5, 7) = ${fn:substring(string,5,7)}"/>
			</li>
		</c:when>
		<c:when test="${step eq '18'}">
			<li>
				...
				<c:set var="string" value="2022-01-01"/>
				<c:out value="fn:replace(\"2022-01-01\", '-', '.') = ${fn:replace(string,'-', '.')}"/><br>
				<c:out value="fn:replace(\"2022-01-01\", '-', '/') = ${fn:replace(string,'-', '/')}"/>
			</li>
		</c:when>
		<c:when test="${step eq '19'}">
			<li>
				<h3>fn:trim() : 문자열의 양쪽 끝에서 공백을 제거</h3>
				주의할 점 : HTML은 공백이 연이어 들어오면 하나로 취급한다.
				<c:set var="str1" value="     Welcome to JSP          programming          "/>
				<p>String_1 Length is ${fn:length(str1)}</p>
				
				<c:set var="str2" value="${fn:trim(str1)}"/>
				<p>String_2 Length is ${fn:length(str2)}</p>
				<p>Final value of string is : ${str2}</p>
				<p>Final value of string is :      Welcome to JSP&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;programming          -</p>
				<a href="#">       ssss           ssssss        aa</a>
			</li>
		</c:when>
